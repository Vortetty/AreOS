...
extern "C" void kernel_main(void) 
{
    ...
    terminal_writestring("Hello world!\nRunning -- <OS_NAME> -- kernel v-- <VERSION> --\n");

    for (char i = 'a'; i <= 'z'; i++) {
        char tmp[2] = {i, '\n'};
        terminal_writestring(tmp);
    }
    //terminal_writestring("ABCDEFGHIJKLMNOPQRSTUBWXYZ !@#$%^&*()_+{}|:\"<>?\nabcdefghijklmnopqrstuvwxyz 1234567890-=[]\\;',./\n");

    //while (1) {
    //    terminal_putentryat('A', VGA_COLOR_LIGHT_GREEN, VGA_WIDTH-1, VGA_HEIGHT-1);
    //    for (uint64_t i = 0; i < 0x7ffffff; i++) asm(" ");
    //    terminal_putentryat('A', VGA_COLOR_LIGHT_RED, VGA_WIDTH-1, VGA_HEIGHT-1);
    //    for (uint64_t i = 0; i < 0x7ffffff; i++) asm(" ");
    //}
}